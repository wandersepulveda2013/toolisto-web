...
toolPages.forEach(file => {
  ...
  if (!tool.enabled) { error(`${slug}: tool is disabled but page exists`); }
  ...
  else if (!canonical.endsWith('/' + slug)) error(`${slug}: canonical does not end with own slug: ${canonical}`);
  ...
  if (!inSitemap && tool.enabledInSitemap) error(`${slug}: not in sitemap.xml`);
  else if (inSitemap) pass(`in sitemap.xml`);

  if (tool.enabledInSitemap) indexablePages.push(slug);
  allSlugs.push(slug);
});
...
catPages.forEach(file => {
  ...
  const canonical = extractCanonical(html);
  if (!canonical) error(`${slug}: missing canonical`);
  else if (!canonical.endsWith('/' + slug)) error(`${slug}: canonical does not end with slug: ${canonical}`);
  else pass(`canonical: ${canonical}`);
...
const extraInSitemap = sitemapUrls.filter(u => !allSlugs.some(s => u === site.siteUrl + '/' + s) && u !== site.siteUrl + '/');
...
if (robots.includes('Disallow')) warn('robots.txt contains Disallow rules');
else pass('robots.txt allows all');
