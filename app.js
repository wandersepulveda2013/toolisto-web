(() => {
  ...
  const toolMeta = {
    ...
    colorPicker: { icon: '🎨', title: 'Extraer colores', description: 'Analiza la paleta de colores dominantes de una imagen.', accepts: 'image' },
    imageCompare: { icon: '🔍', title: 'Comparar imágenes', description: 'Detecta diferencias pixel a pixel entre dos imágenes.', accepts: 'images' },
  ...
  function init() {
    ...
    function runSmartSearch() {
      ...
      results.innerHTML = hits.map((hit, i) => {
        ...
        return `<div class="smart-search-item${i === 0 ? ' recommended' : ''}" data-href="${hit.href}" role="link" tabindex="0" aria-label="Abrir ${hit.name}">
          <span class="smart-search-icon" style="background:${color}">${icon}</span>
          <span class="smart-search-info">
            ${recLabel}<span class="smart-search-name">${hit.name}</span>
            <span class="smart-search-desc">${hit.desc}</span>
            ${formatInfo ? `<span class="smart-search-meta">${catLabel} · ${formatInfo}</span>` : `<span class="smart-search-meta">${catLabel}</span>`}
          </span>
  ...
  function renderAdvancedControls(tool) {
    ...
    const htmlByTool = {
      ...
      `,
      qrGenerate: `
        ${controlText('qrText', 'Texto o URL', 'https://toolisto.com', 'Ingresa el contenido del QR')}
      ...
      colorPicker: `
        <div class="control" style="grid-column:1/-1;color:var(--muted);font-size:.85rem">
          Se analizarán los 10 colores más frecuentes de la imagen y se exportará la paleta completa.
        </div>
      `,
      imageCompare: `
        <div class="control" style="grid-column:1/-1;color:var(--muted);font-size:.85rem">
          Compara las dos imágenes píxel a píxel y genera una imagen de diferencias en rojo.
        </div>
        ${controlNumber('diffThreshold', 'Umbral de diferencia', 30, 1, 128)}
      `,
      pdfPageCounter: `
        <div class="control" style="grid-column:1/-1;color:var(--muted);font-size:.85rem">
      ...
      cameraDocumentScanner: `
        ${controlSelect('camOutputFormat', 'Formato de salida', [['image/jpeg','JPG'],['image/png','PNG'],['image/webp','WebP'],['application/pdf','PDF']])}
        ${controlNumber('camQuality', 'Calidad (%)', 92, 25, 100)}
      ...
      `,
      pdfTablesToExcel: `
        ${controlSelect('pdfTableOutput', 'Formato de salida', [['xlsx','Excel (XLSX)'],['csv','CSV']])}
        <div class="control" style="grid-column:1/-1;color:var(--muted);font-size:.82rem;font-style:italic">
          Nota: Esta herramienta funciona de manera óptima con tablas simples. Resultados variables con tablas complejas.
        </div>
      `,
      imageTableToExcel: `
        ${controlSelect('imgTableLang', 'Idioma del texto', [['eng','Inglés'],['spa','Español'],['por','Portugués'],['fra','Francés'],['deu','Alemán']])}
        ${controlSelect('imgTableOutput', 'Formato de salida', [['xlsx','Excel (XLSX)'],['csv','CSV']])}
        <div class="control" style="grid-column:1/-1;color:var(--muted);font-size:.82rem;font-style:italic">
          Nota: La precisión del OCR depende de la calidad de la imagen. Imágenes claras producen mejores resultados.
        </div>
      `,
      convertAudio: `
