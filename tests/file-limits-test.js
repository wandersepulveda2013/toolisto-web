...
test('file-limits.js loaded in Toolisto catalogue', () => {
  const homepage = fs.readFileSync(join(ROOT, 'dist', 'toolisto.html'), 'utf-8');
  assert(homepage.includes('file-limits.js'), 'file-limits.js not found in Toolisto catalogue');
});
...
// --- Module J: Signature validation (TLT-041) ---
console.log('\n--- 10. Signature Validation ---');

function makeFileWithHeader(name, type, hex) {
  ...
  const file = {
    ...
    size: bytes.length,
    slice(start, end) {
      const chunk = bytes.slice(start || 0, end);
      return {
        arrayBuffer() {
          return Promise.resolve(chunk.buffer.slice(chunk.byteOffset, chunk.byteOffset + chunk.byteLength));
        }
      };
    }
  };
...
function readHeaderHexFromFile(file) {
  return file.slice(0, 16).arrayBuffer().then(buf => {
...
test('detectSignature recognizes PDF magic bytes', () => {
  const FL = loadFileLimits(createSandbox());
  assertEqual(FL.detectSignature('255044462d312e34'), 'PDF');
});

test('detectSignature recognizes PNG magic bytes', () => {
  const FL = loadFileLimits(createSandbox());
  assertEqual(FL.detectSignature('89504e470d0a1a0a'), 'PNG');
});

test('detectSignature recognizes JPEG magic bytes', () => {
  const FL = loadFileLimits(createSandbox());
  assertEqual(FL.detectSignature('ffd8ffe000104a464946'), 'JPEG');
});

test('detectSignature recognizes ZIP magic bytes', () => {
  const FL = loadFileLimits(createSandbox());
  assertEqual(FL.detectSignature('504b030414000000'), 'ZIP/DOCX/XLSX/EPUB');
});

test('detectSignature returns empty for unknown', () => {
  const FL = loadFileLimits(createSandbox());
  assertEqual(FL.detectSignature('deadbeef'), '');
});

test('verifySignature accepts real PDF content', async () => {
  const FL = loadFileLimits(createSandbox());
  const file = makeFileWithHeader('doc.pdf', 'application/pdf', '255044462d312e34');
  const res = await FL.verifySignature(file);
  assertEqual(res.ok, true);
  assertEqual(res.detected, 'PDF');
});

test('verifySignature rejects text disguised as PDF', async () => {
  const FL = loadFileLimits(createSandbox());
  const file = makeFileWithHeader('fake.pdf', 'application/pdf', 'efbbbf48656c6c6f');
  const res = await FL.verifySignature(file);
  assertEqual(res.ok, false);
  assertEqual(res.detected, 'TXT-UTF8');
  assert(res.reason.includes('no coincide'), 'reason explains mismatch');
});

test('verifySignature rejects JPEG named as PNG', async () => {
  const FL = loadFileLimits(createSandbox());
  const file = makeFileWithHeader('fake.png', 'image/png', 'ffd8ffe000104a464946');
  const res = await FL.verifySignature(file);
  assertEqual(res.ok, false);
  assertEqual(res.detected, 'JPEG');
});

test('verifySignature accepts real PNG content', async () => {
  const FL = loadFileLimits(createSandbox());
  const file = makeFileWithHeader('img.png', 'image/png', '89504e470d0a1a0a0000000d');
  const res = await FL.verifySignature(file);
  assertEqual(res.ok, true);
});

test('verifySignature accepts unknown/unspecified formats', async () => {
  const FL = loadFileLimits(createSandbox());
  const file = makeFileWithHeader('data.bin', 'application/octet-stream', 'deadbeefcafe');
  const res = await FL.verifySignature(file);
  assertEqual(res.ok, true, 'No expected signature => not blocked');
});

test('verifySignature accepts real DOCX (zip) content', async () => {
  const FL = loadFileLimits(createSandbox());
  const file = makeFileWithHeader('doc.docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', '504b030414000000');
  const res = await FL.verifySignature(file);
  assertEqual(res.ok, true);
});

test('app.js wires verifyAcceptedSignatures into addFiles', () => {
  const appSrc = fs.readFileSync(join(ROOT, 'app.js'), 'utf-8');
  assert(appSrc.includes('verifyAcceptedSignatures'), 'app.js defines verifyAcceptedSignatures');
  assert(appSrc.includes('verifySignature(file)'), 'app.js calls verifySignature');
});
