...
(async () => {
  ...
  const cardToolIds = await page.$$eval('.tool-card', els => els.map(el => el.getAttribute('data-tool')).filter(Boolean));
  console.log(`  Cards found: ${cardToolIds.length}`);
  ...
  for (const tool of toolsJson) {
    ...
    // 2a. Card exists on homepage
    report.checks.card = cardToolIds.includes(tool.toolId);
