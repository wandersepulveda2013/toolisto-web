...
console.log('=== Toolisto Catalogue Audit Test ===\n');
...
if (!existsSync(distToolisto)) { fail('dist/toolisto.html no existe — ejecuta npm run build'); process.exit(1); }
const html = readFileSync(distToolisto, 'utf8');

// Count tool cards (solo habilitadas)
const cardCount = (html.match(/class="tool-card"/g) || []).length;
if (cardCount !== enabledTools.length) {
  fail(`Catálogo Toolisto: ${cardCount} tarjetas vs ${enabledTools.length} herramientas habilitadas`);
} else {
  pass(`Catálogo Toolisto: ${cardCount} tarjetas = ${enabledTools.length} herramientas habilitadas`);
}
...
if (missingCards === 0) pass(`Todas las ${enabledTools.length} herramientas habilitadas tienen tarjeta`);
...
if (strayCards === 0) pass(`Ninguna de las ${disabledTools.length} herramientas en revisión tiene tarjeta`);
