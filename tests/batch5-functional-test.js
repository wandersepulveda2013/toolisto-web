...
ok(`Total tool count is ${toolsJson.length} (≥144)`, toolsJson.length >= 144);
ok(`Habilitadas=${toolsJson.filter(t => t.enabled).length} / en revisión=${toolsJson.filter(t => !t.enabled).length}`, toolsJson.filter(t => t.enabled).length + toolsJson.filter(t => !t.enabled).length === toolsJson.length);

const EXPECTED_ENABLED = Object.fromEntries(NEW_TOOLS.map(t => [t.toolId, true]));

for (const t of NEW_TOOLS) {
  const tool = toolsJson.find(x => x.toolId === t.toolId);
  ok(`${t.toolId} exists in tools.json`, !!tool);
  if (tool) {
    const expected = EXPECTED_ENABLED[t.toolId] === true;
    ok(`${t.toolId} enabled=${expected} (matriz)`, tool.enabled === expected);
...
if (calcCat) {
  ...
  ok('calculators incluye al menos las 2 herramientas certificadas', calcCat.toolIds.length >= 2);
}
...
// ═══════════════════════════════════════════════════
// 8. toolisto.html Tool Cards (7 new cards)
// ═══════════════════════════════════════════════════
console.log('\n=== 8. toolisto.html Tool Cards ===\n');

const indexHtml = readDist('toolisto.html');
const enabledCount = toolsJson.filter(t => t.enabled).length;

for (const t of NEW_TOOLS) {
  const enabled = EXPECTED_ENABLED[t.toolId] === true;
  if (enabled) {
    ok(`toolisto.html has tool card for ${t.toolId}`, indexHtml.includes(`data-tool="${t.toolId}"`));
  } else {
    ok(`toolisto.html has NO tool card for en-revisión ${t.toolId}`, !indexHtml.includes(`data-tool="${t.toolId}"`));
    const pageHtml = distExists(t.slug + '.html') ? readDist(t.slug + '.html') : '';
    ok(`${t.slug}.html noindex`, pageHtml.includes('<meta name="robots" content="noindex, nofollow">'));
    ok(`${t.slug}.html review notice`, pageHtml.includes('id="toolDisabledNotice"'));
  }
}

const cardCount = (indexHtml.match(/class="tool-card"/g) || []).length;
  ok(`Total tool cards = ${enabledCount} (found ${cardCount})`, cardCount === enabledCount);

ok('toolisto.html has calculators filter chip', indexHtml.includes('data-filter="calculators"'));
...
ok('calculadoras.html enlaza calculadora-simple.html habilitada', calcHtml.includes('calculadora-simple.html'));
ok('calculadoras.html enlaza calculadora-cientifica.html habilitada', calcHtml.includes('calculadora-cientifica.html'));
