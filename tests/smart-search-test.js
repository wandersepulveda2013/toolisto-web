...
(async () => {
  ...
  console.log('\n--- 3. Natural language: "quiero unir varios archivos txt" ---');
  const mergeResults = await page.evaluate(() => window.ToolistoSearch.search('quiero unir varios archivos txt'));
  ok('Returns results', mergeResults.length > 0);
  ok('Top result is mergeTxt', mergeResults[0]?.toolId === 'mergeTxt');

  console.log('\n--- 4. Natural language: "comprimir varias imágenes" ---');
  const sigResults = await page.evaluate(() => window.ToolistoSearch.search('comprimir varias imágenes'));
  ok('Returns results', sigResults.length > 0);
  ok('Top result is batchCompress', sigResults[0]?.toolId === 'batchCompress');
  ...
  console.log('\n--- 6. Natural language: "pasar un documento word a txt" ---');
  const w2pResults = await page.evaluate(() => window.ToolistoSearch.search('pasar un documento word a txt'));
  ok('Returns results', w2pResults.length > 0);
  ok('Top result is wordToTxt', w2pResults[0]?.toolId === 'wordToTxt');
  ...
  console.log('\n--- 8. Synonyms: "unir txt" === mergeTxt ---');
  const synResults = await page.evaluate(() => window.ToolistoSearch.search('unir txt'));
  ok('Returns results', synResults.length > 0);
  ok('"unir txt" resolves to mergeTxt', synResults[0]?.toolId === 'mergeTxt');

  console.log('\n--- 9. Synonyms: "juntar txt" === mergeTxt ---');
  const synResults2 = await page.evaluate(() => window.ToolistoSearch.search('juntar txt'));
  ok('Returns results', synResults2.length > 0);
  ok('"juntar txt" resolves to mergeTxt', synResults2[0]?.toolId === 'mergeTxt');
  ...
  console.log('\n--- 11. Typo tolerance: "unir tx" (truncated) ---');
  const truncResults = await page.evaluate(() => window.ToolistoSearch.search('unir tx'));
  ok('Returns results for truncated query', truncResults.length > 0);
  ok('Top result is mergeTxt', truncResults[0]?.toolId === 'mergeTxt');

  console.log('\n--- 12. Similar actions differentiated ---');
  const splitResults = await page.evaluate(() => window.ToolistoSearch.search('separar un archivo txt'));
  ok('"Separar un archivo txt" returns splitTxt', splitResults.length > 0);
  ok('Top result is splitTxt', splitResults[0]?.toolId === 'splitTxt');

  console.log('\n--- 13. EPUB image tools now available ---');
  const videoGifResults = await page.evaluate(() => window.ToolistoSearch.search('extraer imágenes epub'));
  ok('EPUB images search returns results', videoGifResults.length > 0);
  ok('Top result is imagesEpub', videoGifResults[0]?.toolId === 'imagesEpub');
  ...
  const xlsxResults = await page.evaluate(() => window.ToolistoSearch.search('excel a html'));
  ok('Returns results for "excel a html"', xlsxResults.length > 0);
  ok('Top result is excelToHtml', xlsxResults[0]?.toolId === 'excelToHtml');
  ...
  const nameResults = await page.evaluate(() => window.ToolistoSearch.search('word a txt'));
  ok('Result has correct name', nameResults[0]?.name === 'Word a TXT');
