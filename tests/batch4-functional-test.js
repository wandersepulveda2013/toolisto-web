...
(async () => {
  ...
  // Batch4 tools are en revisión (matrix): página conservada pero sin tarjeta, noindex y bloqueada.
  ...
  for (const t of NEW_TOOLS) {
    ...
    ok(`No card for en-revisión tool: ${t.toolId}`, !card);
    const htmlPath = join(DIST, t.slug + '.html');
    const pageHtml = existsSync(htmlPath) ? readFileSync(htmlPath, 'utf8') : '';
    ok(`Page kept for ${t.toolId}: ${t.slug}.html`, existsSync(htmlPath));
    ok(`${t.toolId} page is noindex`, pageHtml.includes('<meta name="robots" content="noindex, nofollow">'));
    ok(`${t.toolId} page shows review notice`, pageHtml.includes('id="toolDisabledNotice"'));
    ok(`${t.toolId} page config enabled=false`, pageHtml.includes('"enabled":false'));
  }

  const totalCards = await page.$$eval('.tool-card[data-tool]', cards => cards.length);
  ok(`Total tool cards = ${enabledCount} (enabled)`, totalCards === enabledCount);
  ...
  for (const slug of SAMPLE_PAGES) {
    ...
    ok(`${slug}: NOT in sitemap.xml (en revisión)`, !sitemap.includes(slug));
  ...
  for (const t of NEW_TOOLS) {
    ...
    if (tool) {
      ...
      ok(`${t.toolId}: enabled = false (en revisión)`, tool.enabled === false);
      ok(`${t.toolId}: indexable = true (página conservada)`, tool.indexable === true);
    }
  }
