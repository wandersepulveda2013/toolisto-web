...
// digitos que el lector PDF trunca a \ddd y corrompe el string.
const WINANSI_HIGH_CP = {
...
function pdfString(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    else if (c <= 255) parts.push('\\' + c.toString(8).padStart(3, '0')); // 1 byte WinAnsi: octal \ddd siempre valido
    else parts.push(WINANSI_HIGH_CP[c] !== undefined ? '\\' + WINANSI_HIGH_CP[c].toString(8).padStart(3, '0') : ' '); // charCodeAt supera 0xFF: re-mapear a su byte WinAnsi si existe, si no espacio no destructivo (nunca \d{4,} que el lector trunca a 3 y corrompe el string)
  }
  return '(' + parts.join('') + ')';
}
