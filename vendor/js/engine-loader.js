...
(function () {
  ...
  var FFMPEG_PATHS = {
    ...
    util: BASE + 'vendor/ffmpeg/util/util.js',
  ...
  var TESSERACT_STATE = { status: 'idle', workers: {}, promise: null, lastUsed: {}, idleTimer: null, idleTimeoutMs: 600000 };
  ...
  function doLoadFFmpeg(onProgress) {
    ...
    FFMPEG_STATE.promise = (function () {
      ...
      // El import() dinámico dentro de un script clásico resuelve rutas relativas
      // contra la URL del script (vendor/js/), no contra el documento. Se absolutiza
      // con new URL(..., document.baseURI) para que apunte a la raíz del sitio.
      var absPath = function (p) {
        return new URL(p, document.baseURI).href;
      };

      progress('Cargando módulo FFmpeg...');
      return import(/* @vite-ignore */ absPath(FFMPEG_PATHS.ffmpeg)).then(function (ffmpegMod) {
        progress('Cargando utilidades...');
        return import(/* @vite-ignore */ absPath(FFMPEG_PATHS.util)).then(function (utilMod) {
  ...
  function resolveLangDataUrl() {
    try {
      return new URL(TESSERACT_PATHS.langDataFallback, window.location.href).href;
    } catch (err) {
      return TESSERACT_PATHS.langDataFallback;
    }
  }

  function pickLangPath(lang) {
    var localBase = resolveLangDataUrl();
    var gz = localBase + '/' + lang + '.traineddata.gz';
    return fetch(gz, { method: 'HEAD' }).then(function (res) {
      return res.ok ? localBase : TESSERACT_PATHS.langData;
    }).catch(function () {
      return TESSERACT_PATHS.langData;
    });
  }
  ...
  function doLoadTesseract(lang, onProgress) {
    ...
    if (TESSERACT_STATE.workers[workerKey]) {
      TESSERACT_STATE.lastUsed[workerKey] = Date.now();
    ...
    TESSERACT_STATE.promise = (function () {
      ...
      function progress(msg) {
        ...
        return loadTesseractLib().then(function () {
          ...
          progress('Localizando datos de idioma...');
          return pickLangPath(lang).then(function (langPath) {
            progress('Creando worker de OCR...');
            // OEM 3 (DEFAULT) = LSTM + legacy: Tesseract elige el mejor por bloque.
            // Mejora el OCR de escaneos pequenos y ruidosos (fixture dificil: 74%->76%
            // chars y 39%->43% words) sin degradar el limpio (se mantiene 100/100).
            return window.Tesseract.createWorker(lang, 3, {
  ...
  /* ── Liberación por inactividad (memoria en sesiones largas) ────────── */

  function terminateWorker(lang) {
    var key = lang || null;
    var keys = key ? [key] : Object.keys(TESSERACT_STATE.workers);
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i];
      var worker = TESSERACT_STATE.workers[k];
      try {
        if (worker && typeof worker.terminate === 'function') worker.terminate();
      } catch (e) {}
      delete TESSERACT_STATE.workers[k];
      delete TESSERACT_STATE.lastUsed[k];
    }
    if (Object.keys(TESSERACT_STATE.workers).length === 0) {
  ...
  function startIdleReaper() {
    if (TESSERACT_STATE.idleTimer || TESSERACT_STATE.idleTimeoutMs <= 0) return;
    var interval = Math.max(1000, TESSERACT_STATE.idleTimeoutMs / 4);
    TESSERACT_STATE.idleTimer = setInterval(reapIdleWorkers, interval);
  }

  function stopIdleReaper() {
    if (TESSERACT_STATE.idleTimer) {
      clearInterval(TESSERACT_STATE.idleTimer);
      TESSERACT_STATE.idleTimer = null;
    }
  }
  ...
  function reapIdleWorkers() {
    ...
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i];
      var idle = now - (TESSERACT_STATE.lastUsed[k] || 0);
      if (idle >= TESSERACT_STATE.idleTimeoutMs) {
        terminateWorker(k);
      } else {
        nextKeys.push(k);
      }
    }
    if (nextKeys.length === 0) stopIdleReaper();
  }
  ...
  function releaseIdleWorkers() {
    ...
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i];
      if (now - (TESSERACT_STATE.lastUsed[k] || 0) >= TESSERACT_STATE.idleTimeoutMs) {
        terminateWorker(k);
      }
    }
  }
  ...
  window.EngineLoader = {
    ...
    // Memoria en sesiones largas: establece la ventana de inactividad tras la
    // cual un worker OCR cargado es terminado y liberado automaticamente.
    setTesseractIdleTimeout: function (ms) {
      var value = Number(ms);
      TESSERACT_STATE.idleTimeoutMs = value > 0 ? value : 0;
      if (value > 0) {
        startIdleReaper();
      } else {
        stopIdleReaper();
    ...
    // Libera ya el/los worker(s) que llevan inactivos, dejando vivos los que
    // se han usado dentro de la ventana configurada. Sin workers: no-op.
    releaseIdleTesseract: function () {
      releaseIdleWorkers();
    },

    // Liberacion inmediata del worker de un idioma (o de todos si no se pasa).
    releaseTesseract: function (lang) {
      terminateWorker(lang);
    },

    getTesseractStatus: function () {
      var langs = [];
      for (var k in TESSERACT_STATE.workers) {
        if (Object.prototype.hasOwnProperty.call(TESSERACT_STATE.workers, k)) langs.push(k);
      }
      return {
        status: TESSERACT_STATE.status,
        langs: langs,
        idleTimeoutMs: TESSERACT_STATE.idleTimeoutMs,
        reaperActive: TESSERACT_STATE.idleTimer !== null
      };
    },
