...
﻿(function () {
  var qrFactory = (typeof window !== 'undefined' && window.qrcode) || null;
  if (!qrFactory) return;

  function buildMatrix(text, ec) {
    var qr = qrFactory(0, ec || 'M');
    ...
    for (var r = 0; r < n; r++) {
      matrix[r] = [];
      for (var c = 0; c < n; c++) matrix[r][c] = qr.isDark(r, c) ? 1 : 0;
    }
    return matrix;
  }
  ...
  window.QRCodeGenerator = {
    generate: function (text, options) {
      ...
      var matrix = buildMatrix(text, opts.errorCorrection);
    ...
    toPNG: function (text, options) { return window.QRCodeGenerator.generate(text, options).toPNG(); },
    toSVG: function (text, options) { return window.QRCodeGenerator.generate(text, options).toSVG(); },
    getDataURL: function (text, options) { return window.QRCodeGenerator.generate(text, options).getDataURL(); }
  };
})();
