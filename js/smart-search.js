;(function () {
  ...
  var ACTIONS = {
    ...
    csvToMarkdown: ['csv a markdown', 'csv a md', 'tabla markdown', 'csv tabla markdown'],
    csvToHtml: ['csv a html', 'csv a tabla html', 'tabla html desde csv'],
    csvToYaml: ['csv a yaml', 'csv a configuración', 'csv a yml'],
    excelToHtml: ['excel a html', 'xlsx a html', 'hoja de cálculo a html'],
    excelToMarkdown: ['excel a markdown', 'xlsx a md', 'excel a tabla markdown'],
    xmlToExcel: ['xml a excel', 'xml a xlsx', 'convertir xml a excel'],
    csvStatistics: ['estadísticas csv', 'analizar csv', 'métricas csv', 'analizar datos csv'],
    csvFilter: ['filtrar csv', 'filtro csv', 'csv por condición', 'filtrar datos'],
    csvSort: ['ordenar csv', 'sort csv', 'ordenar por columna', 'csv ordenado'],
    csvToSql: ['csv a sql', 'csv a base de datos', 'csv insert sql', 'create table csv'],
    jsonFormatter: ['formatear json', 'json pretty', 'json bonito', 'embellecer json', 'compactar json'],
    excelToXml: ['excel a xml', 'xlsx a xml', 'hoja de cálculo a xml'],
    jsonValidator: ['validar json', 'comprobar json', 'json válido', 'verificar json'],
  ...
  function buildIndexFromData(data) {
    ...
    (data || []).forEach(function (entry) {
      ...
      var desc = String(entry.summary || entry.desc || '');
      var href = String(entry.href || ('/' + (entry.slug || '')));
      var inputFmt = String(entry.inputFmt || entry.inputFormats || '');
      var outputFmt = String(entry.outputFmt || entry.outputFormats || '');
      var keywords = String(entry.keywords || '');
      var actions = ACTIONS[toolId] || [];
  ...
  function search(query, maxResults) {
    ...
    var maxResults = (typeof maxResults === 'number' && maxResults > 0) ? maxResults : 5;
    var results = [];
    var seen = {};
  ...
  window.ToolistoSearch = {
    ...
    buildIndexFromData: buildIndexFromData,
    search: search,
